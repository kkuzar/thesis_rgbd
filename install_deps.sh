...
mkdir build
cd build
cmake -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_FRAMEWORK_INSTALL_PREFIX=$prefix/lib -DIOS_DEVICE_ARCHITECTURES="arm64" -DIOS_SIMULATOR_ARCHITECTURES="iOS-simulator" -DBUILD_EXAMPLES=OFF -DBUILD_TESTING=OFF -DVTK_IOS_BUILD=ON -DModule_vtkFiltersModeling=ON ..
...
echo "git clone rtabmap as source"
git clone https://github.com/introlab/rtabmap.git
mv rtabmap rtab_src
cd rtab_src
git checkout tags/0.21.4.1
cd ..

mkdir rtabmap
cd rtabmap
echo "configure and build rtabmap from source"
cmake -DANDROID_PREBUILD=ON ../rtab_src
cmake --build . --config Release
mkdir ios
cd ios
...
echo "Dependancy building success"
